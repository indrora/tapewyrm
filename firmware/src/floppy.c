...
/*
 * Tapewyrm QIC graft: forward declarations for the hooks rdata_encode_flux() /
 * floppy_read() call into the QIC capture engine. The implementations live in
 * qic/qic.c, which is #included further down (after the GW flux read engine is
 * defined) so it can see GW's static primitives directly. See qic/qic.c.
 *
 * protocol.h (generated) is pulled in here so the marker/END reason codes
 * (TW_*) are visible at these call sites, not just inside qic/qic.c.
 */
#include "protocol.h"
...
{
    /* Tapewyrm: if a tape capture is running, stop the tape before tearing the
     * link down (safety-critical, §5.2). quiesce_drives() handles the floppy
     * flux engine; this handles the QIC motion state. */
    qic_capture_abort_silent();
    floppy_state = ST_inactive;
    quiesce_drives();
    act_led(FALSE);
}
...
static void rdata_encode_flux(void)
{
    ...
    if (read.nr_index != index.count) {
        ...
        _write_28bit(ticks);
        /* Tapewyrm: tape asserts INDEX once per SEGMENT, not per revolution
         * (§2.2). Ride GW's existing index branch to emit a typed SEGMENT
         * marker alongside GW's FLUXOP_INDEX, using GW's own tick delta, so the
         * host gets per-segment boundaries for free (§5.4, §7.1). No-op unless a
         * QIC capture is armed. */
        qic_capture_on_index(ticks);
    ...
    for (; cons != prod; cons = (cons+1) & buf_mask) {
        /* Tapewyrm: remember where this flux value starts so the QIC capture can
         * account the exact DATA bytes (byte_count + checksum) the host will
         * receive (§5.4 END accounting). Cheap and a no-op when not capturing. */
        uint32_t qic_prod0 = u_prod;
        next = dma.buf[cons];
        curr = next - prev;
        prev = next;
        ...
        /* Tapewyrm: account the bytes just emitted for this transition. The
         * accountant is a no-op unless a QIC capture is armed (it checks its own
         * state), so this costs only a compare on the normal GW read path. */
        if (u_prod != qic_prod0) {
            qic_capture_account_range(qic_prod0, u_prod - qic_prod0);
            qic_capture_account_transition();
        }
    }
...
static void floppy_read(void)
{
    ...
    if (floppy_state == ST_read_flux) {
        ...
        /* Tapewyrm: emit SESSION_START at the very front of a capture run, once,
         * before any flux (no-op for a normal GW read or after the first pump). */
        qic_capture_emit_session_start_if_needed();
        ...
        if (avail > U_BUF_SZ) {
            ...
            floppy_flux_end();
            /* Tapewyrm: never silently drop tape flux -- a sustained overflow
             * means the host can't keep up, so stop the tape cleanly (§5.4). The
             * buffer is zeroed below (the partial run can't be trusted across the
             * gap), so we stop the tape but cannot deliver END; the host learns
             * the cause from the ACK_FLUX_OVERFLOW status and retries the pass. */
            qic_capture_abort_silent();
             ...
             * clean stop path: halt the flux front-end, then stop the tape and
             * seal the run with a valid END marker (qic_capture_finish writes END
             * into u_buf[] -- the buffer is NOT zeroed here, so it streams out as
             * the tail of the capture and the host's verify() passes). (§5.4) */
            floppy_flux_end();
            qic_capture_finish(TW_END_NORMAL);
            floppy_state = ST_read_flux_drain;
...
/*
 * Tapewyrm QIC graft. Included HERE -- after the GW flux read engine, pin
 * macros, watchdog, u_buf[]/U_MASK, gw_info and floppy_read_prep() are all
 * defined -- so the QIC code can bind directly to those static GW primitives
 * (DESIGN.md §4: borrow GW's control loop + flux recording). Provides the QIC
 * verbs, the tape markers, and the capture engine (which reuses GW's flux read),
 * plus the qic_cmd_* handlers grafted into process_command() below.
 */
#include "qic/qic.c"
...
static void process_command(void)
{
    ...
    /*
     * Tapewyrm QIC verbs + capture, grafted onto GW's command dispatch. New
     * command numbers (0x80+) in a range clearly separate from GW's 0..CMD_MAX,
     * so a stock `gw` host simply gets ACK_BAD_COMMAND. Handlers live in
     * qic/qic.c. (DESIGN.md §5.3 verbs, §5.4 capture, §13.3 wire protocol.)
     */
    case CMD_QIC_INFO: {
        resp_sz = qic_cmd_info();
        goto out;
    }
    case CMD_QIC_SET_TIMING: {
        qic_cmd_set_timing(len);
        goto out;
    }
    case CMD_QIC_PULSES: {
        resp_sz = qic_cmd_pulses(len);
        goto out;
    }
    case CMD_QIC_WAIT_READY: {
        resp_sz = qic_cmd_wait_ready(len);
        goto out;
    }
    case CMD_QIC_CAPTURE: {
         ...
         * like CMD_READ_FLUX. u_buf[1] carries the floppy_read_prep() status. */
        qic_cmd_capture(len);
        goto out;
    }
...
static void floppy_configure(void)
{
    watchdog_arm();
    floppy_flux_end();
    /* Tapewyrm: floppy_configure() is GW's out-of-band clear-comms / watchdog
     * abort funnel (host stops a stream via BAUD_CLEAR_COMMS -> here, and the
     * command watchdog routes here too). It is exactly the Quiesce trigger for a
     * held capture (§5.2). The flux front-end is now halted (floppy_flux_end
     * above), so STEP is free: stop the tape. The buffer is about to be zeroed,
     * so END can't be delivered -- the run is truncated but still decodes. */
    qic_capture_abort_silent();
    floppy_state = ST_command_wait;
    u_cons = u_prod = 0;
    act_led(FALSE);
